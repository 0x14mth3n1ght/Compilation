...
public:
    ...
    unsigned int execute(function* fun)
    {
        ...
        {   
            ...
            //Q7
            printf("\tBB %02d\n", bb->index);
            auto_vec<basic_block> pdominated = get_all_dominated_blocks(CDI_POST_DOMINATORS, bb);
            for (unsigned int i = 0; i < pdominated.length(); i++) {
                if (bb->index != pdominated[i]->index){
                    printf("\t\t>> %02d\n", pdominated[i]->index);
                }
            }
        }
